//Marta Ferández de la Mela Salcedo
//Clara Palomares Calvo
//PARTE OPCIONAL (incluye la obligatoria)
//Filtro anisotrópico, añadir modelo (pirámide), luces, bump mapping, textura especular
...
glm::mat4   model3 = glm::mat4(1.0f); //PARTE OPCIONAL 2c: Matriz model para la pirámide
...
int inNormal; 
int inTexCoord;
int inTangent; //Guardar identificador de inTangent
...
unsigned int tangentVBO; //Tangentes

// PARTE OPCIONAL:
unsigned int vao2; //// VAO para la pirámide

// VBOs que componen la pirámide
...
unsigned int specularTexId; //Luz especular
unsigned int normalTexId; //Mapa normales
//8.3) Identificadores de las variables globales del shader que permiten acceder a las variables uniformes
int uColorTex;
int uEmiTex;
int uSpecularTex; //Luz especular
int uNormalTex; //Mapa normales
...
int main(int argc, char** argv)
{
	...
	initOGL();
	initShader("../shaders_P3/shader.v1.vert", "../shaders_P3/shader.v2.frag");
	initObj();
...
void destroy()
{
	...
	//PARTE OPCIONAL: Borro los VBOs de la pirámide
	...
	glDeleteVertexArrays(1, &vao2); //Borrar VAO pirámide
...
void initShader(const char *vname, const char *fname)
{
	...
	uSpecularTex = glGetUniformLocation(program, "specularTex"); //Luz especular
	uNormalTex = glGetUniformLocation(program, "normalTex"); //Mapa normales
...
void initObj()
{
	...
	// PARTE OPCIONAL 2c: Inicializar el VAO de la pirámide
	glGenVertexArrays(1, &vao2); //Crear VAO con su identificador
	glBindVertexArray(vao2);	 //Activar el VAO

	//Configurar atributos de VBOs de la pirámide
	if (inPos != -1)
	{
		glGenBuffers(1, &posVBO2);
		glBindBuffer(GL_ARRAY_BUFFER, posVBO2);
		glBufferData(GL_ARRAY_BUFFER, piramNVertex*sizeof(float)* 3,
			piramVertexPos, GL_STATIC_DRAW);
	...
	if (inColor != -1)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, piramNVertex * sizeof(float) * 3,
			piramVertexColor, GL_STATIC_DRAW);
	...
	if (inNormal != -1)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, piramNVertex*sizeof(float)* 3,
			piramVertexNormal, GL_STATIC_DRAW);
	...
	if (inTexCoord != -1)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, piramNVertex*sizeof(float)* 2,
			piramVertexTexCoord, GL_STATIC_DRAW);

		glVertexAttribPointer(inTexCoord, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(inTexCoord);
	}
	
	//Buffer de índices de la pirámide
	glGenBuffers(1, &triangleIndexVBO2); //Crear
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, triangleIndexVBO2); //Activar
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,
		piramNTriangleIndex*sizeof(unsigned int)* 3, piramTriangleIndex,
		GL_STATIC_DRAW); //Subir lista de índices
	...
	model3 = glm::mat4(1.0f); //PARTE OPCIONAL 2c: matriz model de la pirámide
...
unsigned int loadTex(const char *fileName)
{ 
	unsigned char *map; //Crear buffer
	unsigned int w, h;
	map = loadTexture(fileName, w, h); //Devuelve buffer donde la textura se ha almacenado
	...
	//PARTE OPCIONAL 1: Activar el filtrado anisotrópico
...
{	
	...
	if (posLuz != -1)
		glUniform3fv(posLuz, 1, &(cLuz[0])); //Se le dice al shader que la textura que va a utilizar están en determinada ud de textura activa (0)
	...
	//PARTE OPCIONAL 2c: Añadir la pirámide a la escena
	modelView = view * model3; //Matriz model pirámide
	modelViewProj = proj * view * model3;  //Matriz modelview*projection
	normal = glm::transpose(glm::inverse(modelView)); //Matriz normal
	//Datos a la gráfica
	if (uModelViewMat != -1)
		glUniformMatrix4fv(uModelViewMat, 1, GL_FALSE,
	...
	//Activar VAO y decirle que lo pinte
	glBindVertexArray(vao2);
	glDrawElements(GL_TRIANGLES, piramNTriangleIndex * 3,
		GL_UNSIGNED_INT, (void*)0);
...
void resizeFunc(int width, int height)
{
	...
	//Aspect ratio: Crear matriz de proyección perspectiva que consevre las proporciones, apertura=60º, near=0.1, far=100
	...
	//El aspect ratio no se ajusta porque no está definida
...
void idleFunc()
{
	...
	//PARTE OPCIONAL 2c: Movimiento pirámide
	model3 = glm::mat4(1.0f);
	model3 = glm::translate(model3, glm::vec3(0.0f, 2.5f, 0.0f)); //Traslación pirámide
	model3 = glm::scale(model3, glm::vec3(0.6f, 0.6f, 0.6f));  //Escalado
...
void keyboardFunc(unsigned char key, int x, int y)
{
	...
	case 's': //Movimiento hacia atrás (eje Z)
		...
		view = moveView * view;
		break;
	case 'a': //'Pasos' a la derecha (eje Y)
		moveView = glm::translate(moveView, glm::vec3(0.1f, 0.0f, 0.0f));
		view = moveView * view;
		break;
	case 'd': //'Pasos' a la izquierda (eje Y)
		moveView = glm::translate(moveView, glm::vec3(-0.1f, 0.0f, 0.0f));
		view = moveView * view;
		break;
